...
%% ---------- Compute equilibrium points and eigenvalues ----------
for k = 1:nL
    lambda = lambdas(k);
...
ylabel('trace(J)'); 
t=title('Hopf condition check: trace=0, det>0, trace crosses 0','FontName','Helvetica','FontSize',20,'FontWeight','normal');
pos = get(t,'Position'); 
pos(2) = pos(2) + 1.55; 
set(t,'Position',pos);

xlim([0 1]); ylim([-6 4]); xticks(0:0.2:1); yticks(-6:2:4); xtickangle(0); ytickangle(0);
...
xlim([0 1]); ylim([-1 3]); xticks(0:0.2:1); yticks(-1:1:3); xtickangle(0); ytickangle(0);
...
%% ---------- Plot (time domain + phase portrait) ----------
...
xlabel('t'); ylabel('θ, ϕ'); lgd = legend('θ(t)','ϕ(t)'); lgd.Box='off';
xlim([0 200]); ylim([-3 12]); xticks(0:50:200); yticks(-3:3:12); xtickangle(0); ytickangle(0);


ax1.Position = [x_gap 0.567 ax_w ax_h];
t=title('λ=0.579 (<λ_c)');
pos = get(t,'Position');   
pos(2) = pos(2) + 0.05;   
set(t,'Position',pos);
...
[th_grid, ph_grid] = meshgrid(linspace(-1,3,20), linspace(2,10,20));
lam = lam_lo;
dth = alpha.*sin(ph_grid - th_grid) + lam - gamma.*sin(th_grid);
dph = rho.*cos(th_grid) - eta.*sin(ph_grid - phistar);
hq  = quiver(th_grid, ph_grid, dth, dph, 'Color',[0.7 0.7 0.7], ...
             'AutoScaleFactor',2);

% -------- Trajectory --------
htraj = plot(z1(:,1), z1(:,2), 'Color',[0.2216 0.6667 0.7059], 'LineWidth',8);

% -------- Equilibrium point --------
heq   = plot(z1(end,1), z1(end,2), 'ko', 'MarkerFaceColor','k', 'MarkerSize',8);

xlabel('θ'); ylabel('ϕ'); 
xlim([-1 3]); ylim([2 10]); xticks(-1:1:3); yticks(2:2:10); xtickangle(0); ytickangle(0);

ax2.Position = [0.54 0.567 ax_w ax_h];
t = title('Phase portrait');
pos = get(t,'Position');
pos(2) = pos(2) + 0.25;
set(t,'Position',pos);

% -------- Legend: vector field + trajectory + equilibrium --------
...
xlabel('t'); ylabel('θ, ϕ'); lgd = legend('θ(t)','ϕ(t)'); lgd.Box='off';
xlim([0 200]); ylim([1 4]); xticks(0:50:200); yticks(1:1:4); xtickangle(0); ytickangle(0);


ax3.Position = [x_gap 0.053 ax_w ax_h];
t=title('λ=0.614 (>λ_c)');
pos = get(t,'Position');   
pos(2) = pos(2) + 0.05;    
set(t,'Position',pos);
...
% -------- Phase portrait: vector field --------
[th_grid2, ph_grid2] = meshgrid(linspace(0,2,20), linspace(1,4,20));
lam = lam_hi;
dth2 = alpha.*sin(ph_grid2 - th_grid2) + lam - gamma.*sin(th_grid2);
dph2 = rho.*cos(th_grid2) - eta.*sin(ph_grid2 - phistar);
hq2  = quiver(th_grid2, ph_grid2, dth2, dph2, 'Color',[0.7 0.7 0.7], ...
              'AutoScaleFactor',2);

% -------- Trajectory (whole integration orbit) --------
htraj2 = plot(z2(:,1), z2(:,2), 'Color',[0.2216 0.6667 0.7059], 'LineWidth',8);


% -------- Limit cycle: approximated by converged tail segment --------
idxTail = t2 > (Tspan(2) - 100);  % Take the last 100 time units
hlc     = plot(z2(idxTail,1), z2(idxTail,2), 'r', 'LineWidth',8);

xlabel('θ'); ylabel('ϕ'); 
xlim([1 2]); ylim([2 4]); xticks(0:0.5:2); yticks(2:1:4); xtickangle(0); ytickangle(0);

ax4.Position = [0.54 0.053 ax_w ax_h];
t = title('Phase portrait (limit cycle)');
pos = get(t,'Position');
pos(2) = pos(2) + 0.1;
set(t,'Position',pos);

% -------- Legend: vector field + trajectory + limit cycle --------
lgd4 = legend([hq2(1) htraj2 hlc], {'Vector field','Track','Limit cycle'}, 'Location','best');
lgd4.Box = 'off';
